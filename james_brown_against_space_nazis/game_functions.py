...
def draw_screen(screen, settings, y, background):
	#Redesenha a tela a cada passagem pelo laço
		
	rel_y = move_background(background, screen, y)
	
	if rel_y < settings.screen_height:
...
def get_number_nazis_x(settings, nazi_width):
	...
	available_space_x 	= settings.screen_width - (4 * nazi_width)
...
def create_nazi(settings, screen, nazis, nazi_number, row_number):
	...
	nazi.rect.x 		= nazi.x
	nazi.rect.y 		= nazi.rect.height + (4 * nazi.rect.height * row_number)
	nazis.add(nazi)
...
def get_number_rows(settings, ship_height, nazi_height):
	...
	number_rows = int(available_space_y / (5 * nazi_height))
	return number_rows
...
def move_background(background, screen, y):
	
	rel_y = (y % background.get_rect().height) * -1
	screen.blit(background, (0,((rel_y * -1) - background.get_rect().height) * -1))
	return rel_y
